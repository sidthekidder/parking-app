...
.controller('RegistrationCtrl', function($scope, $state, $http, $window, $ionicHistory) {
  ...
  $scope.register = function() {
    ...
    $http({
      url: 'https://sliblra7eac59ae.hana.ondemand.com/parkmyride/createuser.xsjs?inumber='+encodeURIComponent($scope.data.inumber)+'&name='+encodeURIComponent($scope.data.name)+'&carnumber='+encodeURIComponent($scope.data.carnumber),
      method: 'GET',
      headers: {
        'accept': 'application/json'
      }
...
.controller('MapCtrl', function($scope, $state, $http, $window, $interval, $ionicPopover) {
  ...
  if (window.innerHeight < 700)
    document.getElementsByClassName('float-button')[0].style.height = "15%"
  else if (window.innerHeight > 1000)
    document.getElementsByClassName('float-button')[0].style.height = "100px"
  else
    document.getElementsByClassName('float-button')[0].style.height = window.innerHeight/100+"%"
...
//asd
  ...
  $http({
    ...
    masterSensorData.forEach(function (sensor) {
      // create a parking spot for each sensor
      // initially show all spots as filled'
      console.log(sensor.SENSOR_ID + ' = ' + sensor.LATITUDE + ', ' + sensor.LONGITUDE)
      var marker1 = new google.maps.Marker({
